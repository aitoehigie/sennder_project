...
def films():
    ...
    '''
    if requests.head("https://ghibliapi.herokuapp.com/films").headers.get("Etag") != headers_to_fs():
        films_json = fetch_movies_and_actors()
        with open("data.txt", "w") as file:
            file.write(f"{films_json}")
        with open("headers.txt", "w") as file:
            file.write(f'{requests.head("https://ghibliapi.herokuapp.com/films").headers.get("Etag")}')
        return f"{films_json}"
    else:
